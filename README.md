...
### Scripts:
### Ontology.py:
The node to communicate with the reset planner to generate the plan or to stop when the solution found. It also recieves the hint and solution from simulation node. And also peforms query operation based on the request. 

### go_to_point.py:
To move the robot to the goal positon

### reset_planner.py:
To genreate the plan and dispatch to the acition interface.
